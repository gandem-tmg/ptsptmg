<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-3">
            ...
            <a href="{{ route('petugas.permohonan.index') }}" class="hidden shrink-0 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 sm:inline-flex sm:px-4 sm:text-sm">
                Tampilan Kartu
            </a>
        </div>
    </x-slot>
    ...
    <div class="space-y-4 py-2">
        <form method="GET" action="{{ route('petugas.permohonan.daftar') }}" class="filter-toolbar">
            ...
            <x-permohonan-filter-bar :seksis="$seksis" :reset-route="route('petugas.permohonan.daftar')" />
        </form>

        <x-data-table :empty="$permohonans->isEmpty()" empty-message="Belum ada permohonan.">
            <thead>
                <tr>
                    <th class="w-10">No</th>
                    <th>No Tiket</th>
            ...
            <tbody>
                ...
                <tr class="row-clickable" onclick="location.href='{{ route('petugas.permohonan.show', $permohonan) }}'">
                    ...
                    <td class="text-right" onclick="event.stopPropagation()">
                        <a href="{{ route('petugas.permohonan.show', $permohonan) }}" title="Lihat" class="icon-action icon-action-view">
