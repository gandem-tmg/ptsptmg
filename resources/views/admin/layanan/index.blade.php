    ...
    <div class="space-y-4 py-2">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
            ...
            <div class="soft-card p-5">
                ...
                @if(!$layanan->kategori)
                <span class="mb-2 inline-flex items-center gap-1 rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-semibold text-red-700">
                    BELUM DIKATEGORIKAN — tidak tampil di halaman publik
                </span>
                @endif
                <p class="text-xs text-slate-400">{{ $layanan->kode_layanan }} &middot; {{ $layanan->seksi->nama_seksi ?? '-' }} &middot; {{ $layanan->persyaratan->count() }} persyaratan</p>
                ...
                <div class="mt-4 flex items-center gap-1 border-t border-slate-100 pt-3">
                    ...
                    <form method="POST" action="{{ route('admin.layanan.destroy', $layanan) }}" onsubmit="return confirm('Hapus layanan ini?')" class="ml-auto">
                        @csrf
                        @method('DELETE')
