<x-app-layout>
    ...
    <div class="workspace-page"><div class="max-w-3xl">
        <form method="POST" action="{{ route('admin.permohonan.update', $permohonan) }}" class="soft-card p-5 sm:p-6">
            @csrf
            @method('PUT')
            <div class="mb-6 rounded-xl bg-emerald-50 p-4"><p class="text-sm text-emerald-800">Tiket <span class="font-mono font-bold">{{ $permohonan->no_tiket }}</span> · {{ $permohonan->nama_layanan_label }}</p></div>
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    <label for="status" class="field-label">Status permohonan</label>
                    <select name="status" id="status" class="form-input" required>
                        @foreach(\App\Models\Permohonan::STATUS_LABELS as $value => $label)
                            <option value="{{ $value }}" {{ $permohonan->status === $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
